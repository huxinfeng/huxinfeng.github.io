    ...
    <>
      ...
      {inputVal.length > 1 && (
        <div className='mt-8'>
          ...
          {searchResults?.length && searchResults?.length === 1 ? ' 个结果' : ' 个结果'}
          关于 &apos;{inputVal}&apos;
        </div>
      )}
      ...
      <ul>
        {searchResults &&
          searchResults.map(({ item, refIndex }) => <Card href={`/posts/${item.slug}/`} frontmatter={item.data} key={`${refIndex}-${item.slug}`} />)}
      </ul>
    </>
  );
}
